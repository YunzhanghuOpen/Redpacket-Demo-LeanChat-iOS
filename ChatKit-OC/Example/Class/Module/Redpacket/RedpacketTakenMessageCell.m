...
- (void)configureCellWithData:(AVIMTypedMessageRedPacketTaken *)message {
    ...
    if ([message isKindOfClass:[AVIMTypedMessageRedPacketTaken class]]){
        _message = (id)message;
        AnalysisRedpacketModel * rpModel = message.rpModel;
        NSString *tip = @"[云红包]";
        
        if([[RedpacketConfig sharedConfig].redpacketUserInfo.userID isEqualToString:rpModel.sender.userID]) {
            if ([[RedpacketConfig sharedConfig].redpacketUserInfo.userID isEqualToString:rpModel.receiver.userID]) {
                tip = NSLocalizedString(@"你领取了自己的红包", @"你领取了自己的红包");
            }else {
                
                // 当前红包 SDK 不返回用户的昵称，需要 app 自己获取
                NSString * nikeName = rpModel.receiver.userName;
                // 收到了别人抢了我的红包的消息提示
                // XXX 领取了你的红包
            ...
            // 显示我抢了别人的红包的提示
            // 你领取了 XXX 的红包
            tip = [NSString stringWithFormat:@"%@%@%@",NSLocalizedString(@"你领取了", @"领取红包消息"),rpModel.sender.userName,NSLocalizedString(@"的红包", @"领取红包消息结尾")];
        }
